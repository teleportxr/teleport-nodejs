...
class Client {
    constructor(cid,sigSend) {
		...
		// The client's axes standard, learned from its Handshake. Every geometric value sent
		// to it is converted into this frame at encode time. NotInitialized until handshake.
	...
	SendGenericResource(uid)
	{
		...
		const urlOverride=this.avatarUrlOverrides&&this.avatarUrlOverrides.get(uid);
    ...
    receiveHandshake(data)
    {
		...
		// Remember the client's axes standard: every geometric value sent to it is converted
		// into this frame at encode time.
